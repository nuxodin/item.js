...
export function cookies(){
    if (!root) {
        ...
        root.getAll = async function(options) {
            ...
            await cookieStore.getAll(options).then( cookies => {
                cookies.forEach(c => {
                    ...
                    item.asyncHandler.ttl = 10000; // hold for 10 seconds in cache
                    item.asyncHandler.setLocal(c.value);
                    result[c.name] = item;
                });
            });
            return result;
        ...
        cookieStore.addEventListener?.('change', e => { // todo: if two windows open, this will trigger twice
            ...
            e.changed.forEach(c => {
                root.item(c.name).asyncHandler.setLocal(c.value); // triggers item change
            });
        });
