...
data.addEventListener('changeIn', ({target, detail}) => {
    if (detail.add)    console.log(`  ➕ Added: ${target.path.join('.')} → ${detail.add.key}`);
    if (detail.remove) console.log(`  ➖ Removed: ${target.path.join('.')} → ${detail.remove.key}`);
    if (detail.value !== undefined)  console.log(`  📝 Changed: ${target.path.join('.')} = ${JSON.stringify(detail.value)}`);
});
