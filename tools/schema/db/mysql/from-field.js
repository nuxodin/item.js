...
// Note: MySQL string lengths are in bytes — divided by 4 (worst-case utf8mb4, 4 bytes/char)
...
const intRanges = {
    ...
    bigint:    { signed: [-9007199254740991, 9007199254740991], unsigned: [0, 9007199254740991] }, // JS safe
}
const textBytes = {
    tinytext:   255,
    text:       65535,
    mediumtext: 16777215,
    // longtext: omitted — too large to be a useful constraint
}
...
export function schemaFromField(row) {
    ...
    if (type === 'tinyint' && len === 1) {
        ...
        prop.type = 'string'; prop.minLength = Math.floor(len / B); prop.maxLength = Math.floor(len / B)

    } else if (type === 'varchar') {
        prop.type = 'string'; prop.maxLength = Math.floor(len / B)

    } else if (type in textBytes) {
        prop.type = 'string'; prop.maxLength = Math.floor(textBytes[type] / B)

    } else if (type === 'longtext') {
        prop.type = 'string'

    } else if (type === 'binary') {
        prop.type = 'string'; prop.contentEncoding = 'base64'; prop.minLength = len; prop.maxLength = len

    } else if (type === 'varbinary') {
        prop.type = 'string'; prop.contentEncoding = 'base64'; prop.maxLength = len

    } else if (/blob/.test(type)) {
        prop.type = 'string'; prop.contentEncoding = 'base64'
    ...
    } else if (type === 'year')      { prop.type = 'integer'; prop.minimum = 1901; prop.maximum = 2155
    } else if (type === 'json')      { prop.type = 'object'
    } else if (type === 'enum')      { prop.type = 'string'; prop.enum = rawLen.split(',').map(s => s.replace(/'/g, ''))
    } else if (type === 'set')       { prop.type = 'array'; prop.items = { type: 'string', enum: rawLen.split(',').map(s => s.replace(/'/g, '')) }
    ...
    if (row.Extra === 'auto_increment') prop['x-autoincrement'] = true
